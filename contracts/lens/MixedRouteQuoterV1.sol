...
import '@draswap/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@draswap/v3-core/contracts/libraries/SafeCast.sol';
import '@draswap/v3-core/contracts/libraries/TickMath.sol';
import '@draswap/v3-core/contracts/libraries/TickBitmap.sol';
import '@draswap/v3-core/contracts/interfaces/IDraswapV3Pool.sol';
import '@draswap/v3-core/contracts/interfaces/callback/IDraswapV3SwapCallback.sol';
import '@draswap/v3-periphery/contracts/libraries/Path.sol';
import '@draswap/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@draswap/v3-periphery/contracts/libraries/CallbackValidation.sol';
import '@draswap/v2-core/contracts/interfaces/IDraswapV2Pair.sol';

import '../base/ImmutableState.sol';
import '../interfaces/IMixedRouteQuoterV1.sol';
import '../libraries/PoolTicksCounter.sol';
import '../libraries/DraswapV2Library.sol';
...
contract MixedRouteQuoterV1 is IMixedRouteQuoterV1, IDraswapV3SwapCallback, PeripheryImmutableState {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IDraswapV3Pool) {
        return IDraswapV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function getPairAmountOut(
        ...
        (uint256 reserveIn, uint256 reserveOut) = DraswapV2Library.getReserves(factoryV2, tokenIn, tokenOut);
        return DraswapV2Library.getAmountOut(amountIn, reserveIn, reserveOut);
    }

    /// @inheritdoc IDraswapV3SwapCallback
    function draswapV3SwapCallback(
        ...
        IDraswapV3Pool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IDraswapV3Pool pool = getPool(params.tokenIn, params.tokenOut, params.fee);

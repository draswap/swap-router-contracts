...
import '@draswap/v2-core/contracts/interfaces/IDraswapV2Callee.sol';
...
contract TokenValidator is ITokenValidator, IDraswapV2Callee, ImmutableState {
    string internal constant FOT_REVERT_STRING = 'FOT';
    // https://github.com/Draswap/v2-core/blob/1136544ac842ff48ae0b1b939701436598d74075/contracts/DraswapV2Pair.sol#L46
    ...
    function _validate(
        ...
        address pairAddress = DraswapV2Library.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(IDraswapV2Pair.token0.selector));
        ...
        IDraswapV2Pair pair = IDraswapV2Pair(pairAddress);
    ...
    function draswapV2Call(
        address,
        uint256 amount0,
        uint256,
        bytes calldata data
    ) external view override {
        IDraswapV2Pair pair = IDraswapV2Pair(msg.sender);
